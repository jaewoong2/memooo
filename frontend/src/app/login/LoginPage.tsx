...
const LOGIN_URL =
  process.env.NODE_ENV === 'development'
    ? process.env.NEXT_PUBLIC_AUTH_URL || 'http://localhost:3001/api/auth/google/login'
    : 'https://api.bamtoly.com/api/auth/google/login';
...
const ClientLoginPage = ({ accessToken, code, redirectUrl }: Props) => {
  ...
  }, [accessToken, code, redirectUrl, isMount, replace, handleAuthentication, redirectToLogin]);

  return (
    <main className='flex size-full min-h-[calc(100vh-60px)] items-center justify-center'>
      <div className='flex flex-col items-center justify-center gap-2'>
        <Image width={128} height={128} alt='플라워' src={process.env.NEXT_PUBLIC_DEFAULT_IMAGE!} className='size-32' />
